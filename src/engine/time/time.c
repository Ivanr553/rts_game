...
void render_frame_data_on_screen(u32 frames)
{
    ...
    if (!global.time.frame_rate_entity_id)
    {
        ...
        add_sprite_sheet_data(frame_rate_entity->render_item, offset, sprite_size, sprite_sheet_size);
        init_render_item(frame_rate_entity->render_item, pos, size, NULL, NULL);
    ...
    else
    {
        ...
        update_text_item(frame_rate_entity->render_item, frame_data, pos);
    }
}
